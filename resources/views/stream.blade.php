...
@section('content')
    <div
        x-data="{ dragging: false, chatWidth: parseInt(localStorage.getItem('chat-width')) ?? 375, tempChatWidth: 375, startDrag: false, chat: true, mode: window.matchMedia('(prefers-color-scheme: dark)').matches ? '&darkpopout' : '' }"
        x-init="$refs.chat.style.width = chatWidth + 'px'"
        class="fixed w-full h-full inset-0 flex"
    >
        <div x-show="dragging" x-on:mousemove.passive="if (dragging) { tempChatWidth = chatWidth + startDrag - $event.pageX; $refs.chat.style.width = tempChatWidth + 'px' }" x-on:mouseup.passive="dragging = false; chatWidth = tempChatWidth; localStorage.setItem('chat-width', chatWidth)" class="z-10 bg-transparent fixed inset-0 w-full h-full"></div>

        <main class="flex-grow relative">

            <div class="absolute top-0 right-0 mr-4 mt-4 flex space-x-2">
                <div x-on:click="chat = !chat" class="cursor-pointer bg-gray-500 opacity-25 hover:opacity-100 text-white rounded-lg p-2">
                ...
                <div x-on:click="fullscreen()" class="cursor-pointer bg-gray-500 opacity-25 hover:opacity-100 text-white rounded-lg p-2">
            ...
            <div x-show="chat" x-on:mousedown.passive="dragging = true; startDrag = $event.pageX" class="cursor-move h-40 w-8 absolute right-0 top-0 bottom-0 my-auto -mr-4 "></div>
    ...
    <script>
       ...
       function stream() {
            return {
                ...
                toggleChat() { this.chat = !this.chat },
                dragging() {
