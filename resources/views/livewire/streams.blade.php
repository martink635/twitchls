<div>
    ...
    <div class="flex flex-wrap items-center justify-between md:flex-no-wrap">
        ...
        <nav
            class="flex flex-col space-y-2 md:space-y-0 md:flex-row flex-wrap md:items-center text-sm font-bold md:space-x-8 lg:w-2/3">
        ...
        <div x-data="{ open: false }" class="mt-4 lg:mt-0 w-full lg:w-1/3">
            ...
            <div x-on:click.away="open = false" x-on:keydown.escape="open = false"
                class="relative z-40 transition duration-500 rounded-t-lg bg-white">
                <div x-on:click="open = true; $nextTick(() => { $refs.search.focus() });"
                    class="flex justify-between px-4 py-4 text-gray-500 cursor-pointer">
                    <input x-show="open" x-ref="search" wire:model="query" type="text" class="w-full outline-none"
                        wire:keydown.arrow-up="decrementHighlight" wire:keydown.arrow-down="incrementHighlight"
                        wire:keydown.enter="filterByHighlight" x-on:keydown.enter="open = false"
                        x-on:keydown.tab="open = false" x-on:keydown.escape="open = false" />
                ...
                <div x-show="open"
                    ...
                    <div class="flex flex-wrap px-4 pb-4 space-y-4 text-sm font-bold overflow-y-scroll"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mt-4 sm:mt-10">
        ...
        <a href="{{ route('stream', ['stream' => $stream['user_name']]) }}"
            class="cursor-pointer block relative flex flex-col rounded-lg justify-end overflow-hidden ransition duration-150 group">
            ...
            <div class="absolute bg-black text-white w-full left-0 right-0 bottom-0 mb-2 px-4 pt-2 pb-2">
                ...
                <div class="text-sm italic leading-6 tracking-wide text-teal-100 mt-1">
